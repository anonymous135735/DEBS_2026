...
public class Node implements PaxosNode {
	...
	private void update(RingDescription ring) throws IOException, KeeperException, InterruptedException {
		int port = 2000 + (ring.getRingID() * 100) + (nodeID * 10);
		if (ring.getRoles().size() > 1) {
        	port += 1; // Slight adjustment for multi-role nodes
    	}

		int originalPort = port;
    	int attempts = 0;
    	boolean bound = false;
		RingManager rm = new RingManager(this, 0, null, null);
		...
		while (!bound && attempts < 10) {
			try {
				...
				zoos.add(zoo);
				rm = new RingManager(this, ring.getRingID(), addr, zoo, "/ringpaxos");
				ring.setRingManager(rm);
				rm.init();
				bound = true;
				logger.info("Node " + nodeID + " bound to port " + port + " for ring " + ring.getRingID());
			} catch (java.net.BindException e) {
				logger.warn("Port " + port + " in use, trying next port...");
				port += 10; // Try next port in sequence
				attempts++;
				
				if (attempts >= 10) {
					throw new IOException("Could not bind to any port after 10 attempts " + 
										"(original: " + originalPort + ")", e);
				}
			}
    	}
		// ring socket port
	...
	public void stop() throws InterruptedException {
		for (RingDescription r : rings) {
			...
			if (ring != null) {
				...
				// Clean up
				try {
					java.lang.reflect.Field serverField = RingManager.class.getDeclaredField("network");
					serverField.setAccessible(true);
					serverField.set(ring, null);
				} catch (Exception e) {
					// ignore
				}
			}
